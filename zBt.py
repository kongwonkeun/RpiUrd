...
class Bt:
    ...
    def connect(self, target=BT_NAME):
        ...
        #
        #try:
        #    subprocess.call(f'./BT_disable.sh', shell=True)
        #except subprocess.CalledProcessError:
        #    print('BT: disable error')
        #try:
        #    subprocess.call(f'./BT_enable.sh', shell=True)
        #except subprocess.CalledProcessError:
        #    print('BT: enable error')
        #time.sleep(5)
        #
        #----
    ...
    def spp_client(self):
        ...
        #
        #try:
        #    subprocess.call(f'./BT_pairing.sh {self.addr}', shell=True)
        #except subprocess.CalledProcessError:
    ...
    def run(self):
        ...
        while self.f:
            try:
                ...
                print(f'BT: sock rx error = {e}')
                self.sock.close()
                time.sleep(1)
                self.sock = bluetooth.BluetoothSocket(bluetooth.RFCOMM)
                self.sock.connect((G_host, G_port))
                print('BT: re-connected')
                continue
            except:
                print(f'BT: sock rx error')
                continue
                #break

            try:
                if  zPipe.G_server[zPipe.PIPE_FOO] != None:
                    os.write(zPipe.G_server[zPipe.PIPE_FOO], rx)
            except BrokenPipeError:
                zPipe.G_server[zPipe.PIPE_FOO] = None
                print(f'BT: foo write error')

            try:
                if  zPipe.G_server[zPipe.PIPE_FOOBAR] != None:
                    os.write(zPipe.G_server[zPipe.PIPE_FOOBAR], rx)
            except BrokenPipeError:
                zPipe.G_server[zPipe.PIPE_FOOBAR] = None
                print(f'BT: foobar write error')
